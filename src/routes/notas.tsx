...
function NotasPage() {
  ...
  useEffect(() => {
    supabase.from("audios").select("*").order("published_at", { ascending: false }).then(({ data }) => {
      setItems(data ?? []);
      setLoading(false);
    });
  }, []);
      ...
      <section className="bg-pitch px-4 py-16 text-primary-foreground">
        <div className="mx-auto max-w-4xl">
          ...
          <h1 className="mt-4 font-display text-5xl font-bold uppercase tracking-tight sm:text-6xl">
            Todas las <span className="text-gold">Notas</span>
          </h1>
          <p className="mt-2 text-white/80">El archivo completo de A LA CANCHA.</p>
        </div>
      </section>
      ...
      <section className="px-4 py-12">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-4 text-sm text-muted-foreground">
            {loading ? "Cargando…" : `${filtered.length} ${filtered.length === 1 ? "nota" : "notas"}`}
          </div>

          <div className="mt-6 space-y-4">
            {!loading && filtered.length === 0 && (
              <div className="rounded-xl border border-dashed p-12 text-center text-muted-foreground">
                No encontramos notas que coincidan con "{q}".
              </div>
            )}
            {filtered.map((a) => (
              <article key={a.id} className="rounded-xl border border-border bg-card p-6 shadow-sm">
                ...
                {a.audio_url && <audio controls className="mt-4 w-full" src={a.audio_url} />}
              </article>
            ))}
          </div>
